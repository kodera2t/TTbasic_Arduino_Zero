...
//          Sept.23, 2015, Arduino Zero (SAMD21G18A) supported with I2C OLED
//   dwrite and wait(delay in mill sec) function are supported!
...
// I2C OLED PIN definition. Change to fit your board....
#define OLED_RESET 7
...
char screenMem[168]; //the implementation of frame buffer is referenced from Ben Heck's
...
#define c_getch( ) Serial1.read()
#define c_kbhit( ) Serial1.available()
...
enum{
  ...
  I_IF, I_REM, I_STOP,
  I_INPUT, I_PRINT, I_LET, I_DWRITE, I_WAIT,
  I_COMMA, I_SEMI,
...
unsigned char* iexe(){
  ...
  while(1){
    ...
    case I_NEXT:
      ...
      cip++;
      vstep = (uintptr_t)lstk[lstki - 2];
      var[index] += vstep;

      vto = (uintptr_t)lstk[lstki - 3];
    //   vto = lstk[lstki - 3];
...
//  newline();
  c_puts("TOYOSHIKI TINY BASIC ARDUINO ZERO EDITION"); 
  newline();
...
static void doFrame(byte amount) {
	...
	//display.cleardisplay();
	//lcd.noCursor();
   display.begin(SSD1306_SWITCHCAPVCC, 0x7B>>1);
        display.clearDisplay();
